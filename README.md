...
- **日常列表** —— 日常模式下，侧边栏工作区区域被替换成日常会话列表：点一行打开该会话并确保仍处于日常模式；标题右侧那个加号图标按钮（和工作模式里「添加工作区」用同一个 `IconProjectAddOutline16`、同一套 28px 圆形 hover 样式、同一套边距 —— `--dsh-sidebar-inline-padding` 内缩再 `-4px` 外拉，右边缘对齐，展开时 16px、收起时 18px）等同于点「日常聊天」。
